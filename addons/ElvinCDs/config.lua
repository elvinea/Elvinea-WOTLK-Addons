...
do
  ...
  local sliders = {'opacity', 'width', 'height', 'spacing', 'maxPerColumn'}
  ...
  local function updateUIFrame(self, elapsed)
    ...
    for k, v in pairs(Elvin_Options) do
      if _G[frameName..'_'..k] then
        ...
        elseif k == 'maxPerColumn' then
          local mpc = addon.options.maxPerColumn or 0
          local label = (mpc == 0) and 'Max per Column: Off'
            or L:F('Max per Column: %s', mpc)
          _G[frameName..'_maxPerColumnText']:SetText(label)
          _G[frameName..'_maxPerColumn']:SetValue(mpc)
        elseif k == 'requestText' then
          local eb = _G[frameName..'_requestText']
          if eb and not eb:HasFocus() then
            eb:SetText(addon.options.requestText or '')
          end
        else
  ...
  function submod:onRequestText(box)
    if not box then return end
    local text = box:GetText()
    if not text or text == '' then
      text = 'Please use {spell} on me'
      box:SetText(text)
    end
    Elvin_Options.requestText = text
  end
end
