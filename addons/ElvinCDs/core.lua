...
local windows = {}
-- Tower layout: all spell windows are chained one under another so they
-- move together and reflow automatically when any window grows/shrinks.
local layoutTower, towerAnchor

-- Map a spellId to its class, so windows can be grouped by class in the
-- tower (a class's spells are never split across columns). Cached because
-- classSpells is keyed class -> spellId and we look up the other way.
local spellClassCache = {}
local function getSpellClass(spellId)
  local cached = spellClassCache[spellId]
  if cached then return cached end
  for class, spells in pairs(classSpells) do
    if spells[spellId] ~= nil then
      spellClassCache[spellId] = class
      return class
    end
  end
  -- Custom/unknown spells sort together after the known classes:
  spellClassCache[spellId] = 'ZZZZ'
  return 'ZZZZ'
end
...
local function createWindow(spellId, wName)
  ...
  -- Window size (position is handled centrally by layoutTower, which
  -- chains every window into a single tower):
  wFrame:SetMovable(true)
  wFrame:SetUserPlaced(false)      -- don't let WoW cache/snap the position
  wFrame:SetClampedToScreen(false) -- allow dropping anywhere, incl. centre
  wFrame:SetWidth(options.width or 150)

  -- No title row: the spell name header is gone. Each player bar shows
  -- the spell icon instead (set up in fetchCooldowns), and the tower is
  -- moved with Shift+drag on any bar. Hide the title bar & window icon
  -- and let the bars fill the whole window.
  ...
  title:SetHeight(0.001)
  title:SetScript('OnMouseDown', nil)
  title:SetScript('OnMouseUp', nil)
  title:SetScript('OnClick', nil)

  local barsFrame = _G[wName..'Bars']
  barsFrame:ClearAllPoints()
  barsFrame:SetPoint('TOPLEFT', wFrame, 'TOPLEFT', 0, 0)
  barsFrame:SetPoint('BOTTOMRIGHT', wFrame, 'BOTTOMRIGHT', 0, 0)
...
-- Lay every visible window out as a tower of one or more columns.
--   * Windows are ordered by class so each class stays contiguous.
--   * A column holds up to options.maxPerColumn windows (0 = unlimited);
--     when a class would overflow the column it moves to the next one
--     whole, so a class is never split across columns.
--   * options.growUp    -> columns build upward instead of downward.
--   * options.columnsLeft-> extra columns are added to the left, not right.
-- Only the tower origin (options.towerX/towerY) is stored; everything is
-- positioned relative to it, so the whole tower moves as one and reflows
-- automatically when any window grows (e.g. a second player of a class).
layoutTower = function()
  local list = {}
  for _, w in pairs(windows) do
    if w and w.frame and not w.hidden and w.bars and #w.bars > 0 then
      table.insert(list, w)
    end
  end

  table.sort(list, function(a, b)
    local ca, cb = getSpellClass(a.spellId), getSpellClass(b.spellId)
  ...
  local growUp      = options.growUp
  local columnsLeft = options.columnsLeft
  local maxPer      = tonumber(options.maxPerColumn) or 0
  local vgap        = (options.spacing or 0) + 4
  local hgap        = 8
  local colWidth    = (options.width or 150) + hgap
  local originX     = options.towerX or 0
  local originY     = options.towerY or -200

  -- Group windows into contiguous class-runs:
  local groups, cur, curClass = {}, nil, nil
  for _, w in ipairs(list) do
    local c = getSpellClass(w.spellId)
    if c ~= curClass then
      cur = {}
  ...
  -- Pack class-groups into columns without splitting a group:
  local columns, col, colCount = {}, nil, 0
  for _, g in ipairs(groups) do
    if not col then
      col = {}; table.insert(columns, col); colCount = 0
    elseif maxPer > 0 and colCount > 0 and (colCount + #g) > maxPer then
      col = {}; table.insert(columns, col); colCount = 0
    end
    for _, w in ipairs(g) do table.insert(col, w) end
    colCount = colCount + #g
  end
...
-- Shift+drag on any bar moves the whole tower (there is no title bar
-- to grab anymore). Dragging shifts the tower origin; layoutTower then
-- repositions every window/column, so they all move together.
local function beginTowerDrag(frame)
  local scale = UIParent:GetEffectiveScale()
  local cx, cy = GetCursorPosition()
  frame.dragCX, frame.dragCY = cx / scale, cy / scale
  frame.dragOX = options.towerX or 0
  frame.dragOY = options.towerY or -200
  frame.dragMoved = false
  frame:SetScript('OnUpdate', function(self)
    local s = UIParent:GetEffectiveScale()
    local nx, ny = GetCursorPosition()
    nx, ny = nx / s, ny / s
    local dx, dy = nx - self.dragCX, ny - self.dragCY
    if dx ~= 0 or dy ~= 0 then self.dragMoved = true end
    options.towerX = self.dragOX + dx
    options.towerY = self.dragOY + dy
    if layoutTower then layoutTower() end
  end)
end

local function endTowerDrag(frame)
    ...
    if window then
      ...
      if bars > 0 then
        ...
        for k, b in ipairs(w.bars) do
          ...
          -- Spell icon shown on the right edge of the bar:
          local barIcon = _G[b.name..'Icon']
          local spIcon = select(3, GetSpellInfo(w.spellId))
          if options.icons and spIcon then
            barIcon:SetTexture(spIcon)
            barIcon:SetTexCoord(0.08, 0.92, 0.08, 0.92)
            barIcon:ClearAllPoints()
            barIcon:SetPoint('RIGHT', bar, 'RIGHT', -1, 0)
            local isz = options.height - 2
            barIcon:SetSize(isz, isz)
            barIcon:Show()
            -- Keep the name/timer text clear of the icon:
            name:ClearAllPoints()
            name:SetPoint('TOPLEFT', bar, 'TOPLEFT', 3, 0)
            name:SetPoint('BOTTOMRIGHT', barIcon, 'BOTTOMLEFT', -2, 0)
            timer:ClearAllPoints()
            timer:SetPoint('TOPLEFT', bar, 'TOPLEFT', 3, 0)
            timer:SetPoint('BOTTOMRIGHT', barIcon, 'BOTTOMLEFT', -2, 0)
          else
            barIcon:Hide()
          end
          ...
          -- Shift+drag moves the tower; a normal click keeps the old
          -- behaviour (whisper menu / Ctrl announce / Alt = spell log):
          bar:SetScript('OnMouseDown', function(self, button)
            local cd = cooldowns[w.spellId]
            if button == 'LeftButton' and IsShiftKeyDown()
              and not options.locked and not (cd and cd.locked) then
              self.towerDragging = true
              beginTowerDrag(self)
            end
          end)
          bar:SetScript('OnMouseUp', function(self, button)
            if self.towerDragging then
              self.towerDragging = false
            ...
            if button == 'MiddleButton' then
              ...
              utils.print(options.locked and L['Windows |cff00ff00locked|r']
                or L['Windows |cffff0000unlocked|r'])
              loaded, fetched = false, false
              return
            elseif button == 'LeftButton' and IsAltKeyDown() then
              if addon.Logs then addon.Logs:view(w.spellId) end
              return
            end
        ...
        -- Window height is now just the bars (no title row):
...
function onUpdateBar(spellId, player, elapsed)
  ...
  if utils.update(bar, barName, 1, elapsed) then
    ...
    if timeTick == 0 then
      ...
      -- Ready: use the player's class colour (grey if dead):
      if r then
        _G[barName]:SetStatusBarColor(r, g, b, options.opacity)
      else
        local _, class = UnitClass(unitId)
        local cr, cg, cb = utils.classColor(class)
        _G[barName]:SetStatusBarColor(cr, cg, cb, options.opacity)
      end
    else
      bar.ticking = true
...
do
  ...
  local function openMenu(self, spellId, group)
    ...
    -- Custom request message (used for both the menu label and the
    -- whisper so they match). {spell} is replaced with the spell.
    local reqText = options.requestText or 'Please use {spell} on me'
    local customMsg = reqText:gsub('{spell}', spellName)

    -- Insert first menu element:
    table.insert(menuItems, {
      text = group and L['Use Now'] or customMsg,
      notCheckable = true,
      func = function()
        if group then
          ...
          addon:whisper(self.player, customMsg)
        end
      end
    })
  ...
  function onClickBar(self, spellId)
    ...
    if IsControlKeyDown() then
      ...
      if spell.blind then
        ...
        local reqText = options.requestText or 'Please use {spell} on me'
        addon:shout(L:F('%s use %s on me!', self.player, spellName))
        addon:whisper(self.player, reqText:gsub('{spell}', spellName))
      end
    else
...
do
  ...
  if not defaultOptions then
    defaultOptions = {
      ...
      columnsLeft=false,
      maxPerColumn=0,
      requestText='Please use {spell} on me',
      default=true
    }
